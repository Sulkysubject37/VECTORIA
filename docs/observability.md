...
| `KernelDispatch` | Kernel selection | "Reference" or "SIMD" |
| `NodeExecutionEnd` | Execution finishes | - |

## Python API
Traces are accessible via `Runtime.get_trace()` (to be implemented).
Trace events are returned as a list of `TraceEvent` objects.
